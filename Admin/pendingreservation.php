  ...
  <aside class="main-sidebar">
    ...
    <section class="sidebar">
	  ...
	  <li class="treeview"> 
		...
		<li class="active"><a href="pendingreservation.php"><i class="fa fa-hourglass-1"></i> <span>Pending Status</span></a></li>		
  ...
  <div class="content-wrapper">
          ...
          <div class="box">
            <div class="box-header">
             <center> <h3 class="box-title"><b>Pending Reservation Table</b></h3></center>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
		...
		if(isset($_GET['search'])){
			$search = ($_GET['search']);
			$qry = mysql_query("SELECT * FROM tbl_addguest where status = 'Pending' AND (rcode like '%$search%' 
					or emailaddress like '%$search%' 
					or firstname like '%$search%' or lastname like '%$search%')");
		}else{
			if(isset($_GET['id'])){
				if($_GET['id'] ==  "All Records"){
					$qry = mysql_query("SELECT * FROM tbl_addguest WHERE status = 'Pending'");
				}
			}
			else{
				$qry = mysql_query("SELECT * FROM tbl_addguest WHERE status = 'Pending'");
	...
	//<button type='button' class='btn btn-success btn-sl' data-toggle='modal' data-target='#myModalView'>Accept Reservation</button>

	
				//$getLinks = "SELECT * FROM tbl_addguest where status='Pending'";
				...
				while($rs=mysql_fetch_array($qry)) {
					...
					$receipt = $rs['image'];
					if ($receipt == !null) {
						$status = "<span class='label label-warning'>".$rs['status']."</span> &nbsp; <img src='dist/img/checked.png'>";
					}
					else {
						
						$status = "<span class='label label-warning'>".$rs['status']."</span>";
					}
						echo "<tr>";
						//echo "<td>".$addguest_id."</td>";
					echo "<td>".$rcode."</td>";
					...
					echo "<td>" .$status. "</td>";
					if ($receipt == !null) {
						echo '<td align="center"> <a href=now.php?addguest_id='.$addguest_id.'&email='.$email.'> <input type="submit" name="Yes" value="Accept Reservation" class="btn btn-success btn-large"></a></td>';
					
					}
					else {
						
						echo '<td align="center"> <a href=now.php?addguest_id='.$addguest_id.'&email='.$email.'> <input type="submit" name="Yes" value="Accept Reservation" class="btn btn-success btn-large" disabled></a></td>';
   ...
   echo ' <div class="modal-dialog modal-sm">';
    echo '  <div class="modal-content">';
     echo '   <div class="modal-header">';
      ...
      echo '    <h4 class="modal-title">Reserve Now</h4>';
      echo '  </div>';
      echo '  <div class="modal-body">';
      echo '    <p>Are you sure you want to reserve this Reservation ?</p>';
	  echo '<a href=now.php?addguest_id='.$addguest_id.'&email='.$email.'> <input type="submit" name="Yes" value="Yes" class="btn btn-success btn-large"></a> &nbsp;';
...
location.href = "pendingreservation.php?id=" + $('.a').val();
})
})
</script>
