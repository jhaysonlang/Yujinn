function formValidation(){
	...
	// Check each input in the order that it appears in the form!
	 ...
	 if(inputAlphabet(firstname, "* Please enter characters only")){
		 
		 if(inputAlphabets(lastname, "* Please enter characters only")){
		
		if(lengthDefine(contact, 6, 8)){
...
function inputAlphabets(inputtext, alertMsg){
	var alphaExp = /^[a-zA-Z]+$/;
	if(inputtext.value.match(alphaExp)){
		return true;
	}else{
		document.getElementById('p7').innerText = alertMsg;  //this segment displays the validation rule for name
		//alert(alertMsg);
		inputtext.focus();
		return false;
	}
